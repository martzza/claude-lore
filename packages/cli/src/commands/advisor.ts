...
export async function advisorClaudeMd(opts: { apply?: boolean }): Promise<void> {
  ...
  if (opts.apply) {
    // In a future phase, this would auto-apply safe optimisations.
    // For now, print a note — the spec says read-only.
    console.log("--apply flag noted. Automated fixes are not yet implemented.");
    console.log("Apply the suggestions above manually to CLAUDE.md.");
  }
}
...
export async function advisorSkills(opts: {
  ...
  if (opts.generate) {
    ...
    const stub = [
      `---`,
      `description: ${suggestion?.description ?? `Skill for ${name}`}`,
      `argument-hint: <symbol or task>`,
      ...
      `## When to use`,
      ``,
      `Use this skill when working on ${name.replace(/-/g, " ")}.`,
      ``,
      `## Approach`,
      ``,
      `1. Start with \`reasoning_get\` to load prior decisions for the relevant symbols.`,
      `2. Check \`session_load\` for open deferred work.`,
      `3. Proceed with the task.`,
      `4. If a new decision is made, use \`reasoning_log\` to record it.`,
      ``,
    ].join("\n");
    ...
    if (existsSync(outPath)) {
      ...
      console.log(`Skill stub written to: ${outPath}`);
    }
  }
}
