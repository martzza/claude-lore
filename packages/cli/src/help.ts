...
export const CLI_COMMANDS: CommandHelp[] = [
...

  {
    name: "wiki",
    group: "visualising",
    summary: "Open an interactive HTML wiki for the codebase communities.",
    description:
      "Generates an interactive dark-theme HTML wiki for each detected community: sidebar search, symbol tables with inline filter, callers/callees, risk scores, and linked decisions/risks. Default behaviour opens the wiki in a browser. Use --format md to stream Markdown to stdout instead.",
    usage: [
      "claude-lore wiki",
      "claude-lore wiki --community build",
      "claude-lore wiki --format md",
      "claude-lore wiki --output docs/wiki/",
    ],
    flags: [
      { flag: "--community <name>",  desc: "Only generate wiki for this community name or id" },
      { flag: "--format <fmt>",      desc: "html (default, opens browser) | md (markdown to stdout)" },
      { flag: "--output <dir>",      desc: "Write index.html (or .md files with --format md) to this directory" },
      { flag: "--open",              desc: "Open wiki in browser (same as default without --format md)" },
    ],
    examples: [
      { command: "claude-lore wiki",                         desc: "Open interactive HTML wiki in browser" },
      { command: "claude-lore wiki --format md",             desc: "Stream all community pages as Markdown to stdout" },
      { command: "claude-lore wiki --community build",       desc: "Open wiki filtered to the 'build' community" },
      { command: "claude-lore wiki --output docs/wiki/",     desc: "Write index.html to docs/wiki/" },
    ],
