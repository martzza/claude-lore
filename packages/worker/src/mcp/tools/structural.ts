...
export function registerStructuralTools(server: McpServer): void {
  ...
  server.tool(
    "codegraph_impact",
    "BFS traversal of the call graph to find all symbols affected if the given symbol changes. Returns impact nodes with hop distance.",
    {
    ...
    async ({ symbol, repo, max_hops }) => {
      ...
      // queue carries the current node and its path from the root symbol
      ...
      for (let hop = 1; hop <= maxHops && queue.length > 0; hop++) {
        ...
        // Build a map from callee -> parent queue item for path tracking
  ...
  server.tool(
    ...
    async ({ task, repo }) => {
      ...
      // Take top 3 symbols: exported first, then shortest name
      const topSymbols = [...foundMap.values()]
        .sort((a, b) => b.exported - a.exported || a.name.length - b.name.length)
        .slice(0, 3);

      // For each symbol get callers + callees + reasoning
      const contextItems = await Promise.all(
        topSymbols.map(async (sym) => {
          const [callersRes, calleesRes] = await Promise.all([
            db.execute({
              sql: `SELECT DISTINCT caller FROM call_graph WHERE callee = ? LIMIT 5`,
              args: [sym.name],
            }),
            db.execute({
              sql: `SELECT DISTINCT callee FROM call_graph WHERE caller = ? LIMIT 5`,
              args: [sym.name],
            }),
          ]);
      ...
      // Compute total downstream impact count (sum of callers)
